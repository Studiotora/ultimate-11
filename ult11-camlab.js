...
(function(){
  ...
  let _freeMoveArmed=false;
  function freeMoveFrame(){
    if(!LAB.active){ _freeMoveArmed=false; return; }   // stop spinning when not in the lab
    {
      try{
  ...
  // start the loop only when the lab activates (see camLabStart)
  LAB._armFreeMove=function(){ if(!_freeMoveArmed){ _freeMoveArmed=true; requestAnimationFrame(freeMoveFrame); } };

  /* ---- launch: reuse the friendly flow ---- */
  function camLabStart(){
    LAB.active = true;
    if(LAB._armFreeMove) LAB._armFreeMove();   // start the free-move watcher now, not at boot
